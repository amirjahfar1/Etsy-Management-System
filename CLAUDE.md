...
`update_listing_inventory` (per-variation SKU/price/quantity — needs a `products` array; see `getListingInventory`'s response shape for the exact structure), `update_variation_images` (assign images to variation values) — **write, confirm first**.
